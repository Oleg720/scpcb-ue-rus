	...
	Else
		...
		Select ms\MainMenuTab
			Case 1 ; New game
				...
				Color(255, 255, 255)
				AASetFont fo\Font[1]
				AAText(GraphicWidth/2, y + height / 2, "НОВАЯ ИГРА", True, True) ;NEW GAME
				...
				If SelectedDifficulty\customizable Then
					...
					AAText(x + 200 * MenuScale, y + 255 * MenuScale, "Два слота в инвентаре") ;Two inventory slots
			...
			Case 2 ;load game
				...
				Color(255, 255, 255)
				AASetFont fo\Font[1]
			    AAText(GraphicWidth/2, y + height / 2, "ЗАГРУЗКА", True, True) ;LOAD GAME
			    ...
			    Color(255, 255, 255)				
				AAText(GraphicWidth/2.0,y+536*MenuScale,"Страница "+Int(Max((CurrLoadGamePage+1),1))+"/"+Int(Max((Int(Ceil(Float(SaveGameAmount)/6.0))),1)),True,True) ;Page
			...
			Case 3,5,6,7 ;options
				...
				Color 255,255,255
				If DrawButton(x+20*MenuScale,y+15*MenuScale,width/5,height/2, "ГРАФИКА", False) Then ms\MainMenuTab = 3 ;GRAPHICS
				If DrawButton(x+160*MenuScale,y+15*MenuScale,width/5,height/2, "АУДИО", False) Then ms\MainMenuTab = 5 ;AUDIO
				If DrawButton(x+300*MenuScale,y+15*MenuScale,width/5,height/2, "УПРАВЛЕНИЕ", False) Then ms\MainMenuTab = 6 ;CONTROLS
				If DrawButton(x+440*MenuScale,y+15*MenuScale,width/5,height/2, "ПРОЧЕЕ", False) Then ms\MainMenuTab = 7 ;ADVANCED
				...
				If ms\MainMenuTab = 3 ;Graphics
					...
					FOV = SlideBarFOV+40
					Color 255,255,255
					AAText(x + 20 * MenuScale, y, "Поле зрения (FOV):") ;Field of view:
					Color 255,255,0
			...
			Case 4 ; load map
				...
				Color(255, 255, 255)
				AASetFont fo\Font[1]
				AAText(GraphicWidth/2, y + height / 2, "ВЫБОР КАРТЫ", True, True) ;LOAD MAP
	...
	Repeat
		...
		Else
			...
			ElseIf GfxModeWidths(SelectedGFXMode)>G_viewport_width Then
				Text(40+ 260 + 65, 262 - 55 + 200, "(уменьшено до "+G_viewport_width + "x" + G_viewport_height + ",32)") ;downscaled to
			EndIf
		EndIf
		...
		If DrawButton(LauncherWidth - 275, LauncherHeight - 50, 150, 30, "НОВОВВЕДЕНИЯ", False, False, False) Then ;SEE CHANGELOG
...
Function DrawOptionsTooltip(x%,y%,width%,height%,option$,value#=0,ingame%=False)
	...
	Select Lower2(option$)
		...
		Case "fov"
			txt = Chr(34)+"Поле зрения"+Chr(34)+" (FOV) - задаёт угол обзора для игрока во время игры." ;Field of view"+Chr(34)+" (FOV) is the amount of game view that is on display during a game."
			R = 255
			G = 255
			B = 255
