				...
				Else
					...
					Else
						...
						If e\EventState > 70 * 9.1 And e\EventState - fs\FPSFactor[0] =< 70 * 9.1 Then
							it.Items = CreateItem("Кошелёк", "wallet", EntityX(e\room\Objects[2], True), EntityY(e\room\Objects[2], True), EntityZ(e\room\Objects[2], True)) ;Wallet
								...
								If KillTimer >= 0 Then 
									For i = 0 To 2
										If Distance(EntityX(Collider), EntityZ(Collider), EntityX(e\room\Objects[i], True), EntityZ(e\room\Objects[i], True)) < 250.0 * RoomScale Then
											...
											CameraShake = 1.0
											Kill()
											DeathMSG = SubjectName$ + " был убит тесла-воротами в [УДАЛЕНО]." ; was killed by the Tesla gate at [REDACTED].
										EndIf
									Next
								EndIf
								    ...
								    If EntityDistance(Curr173\Collider, e\room\Objects[4]) < 3.0
									    If (BlinkTimer < -10 Or (Not EntityInView(Curr173\obj, Camera))) And Curr173\Idle = 0 Then
										    ...
										    If (Not WearingHazmat) Then
                                                ...
                                                Else
                                                    Damage1033RU(10 + (Rand(5) * SelectedDifficulty\aggressiveNPCs))
                                                    Msg = "Окно разбилось, и осколок стекла порезал Вам руку, но SCP-1033-RU защитил Вас." ;The window shattered and a piece of glass cut your arm, but SCP-1033-RU protected you.
                                                EndIf
							...
							If (IsItemGoodFor1162(itt)) Then
								Select Inventory(e\EventState2)\itemtemplate\tempname
									...
									Case "scp198"
									    ...
									    LightFlash = 5.0
									    Kill(True)
									    Msg = "SCP-198 отлип от Вашего тела, но теперь у Вас серьёзные травмы." ;SCP-198 is now detached from your body, but you have high injuries.
									    MsgTimer = 70 * 5
							            Exit
									Case "scp357"
					...
					ElseIf e\EventState3 = 2.0
						...
						For itt.ItemTemplates = Each ItemTemplates
							If IsItemGoodFor1162(itt) And Rand(6) = 1
								...
								e\EventState3 = 0.0
								If Injuries > 15
									DeathMSG = "Мёртвый"+SubjectName$+" был обнаружен внутри камеры содержания SCP-1162." ;A dead"+SubjectName$+" was discovered within the containment chamber of SCP-1162."
		        ...
		        If e\EventState = 0 Then
                    If PlayerRoom = e\room Then
                        If Curr096 = Null And RemoteDoorOn Then
				    	    ...
				    	    ;Spawn items
				    	    it = CreateItem("Ключ-карта 2-го уровня", "key2", EntityX(e\room\Objects[2], True), EntityY(e\room\Objects[2], True), EntityZ(e\room\Objects[2], True)) ;Level 2 Key Card
						    EntityType(it\collider, HIT_ITEM)
								
						    it = CreateItem("Неизвестный документ", "paper", EntityX(e\room\Objects[4], True), EntityY(e\room\Objects[4], True), EntityZ(e\room\Objects[4], True)) ;Unknown Document
						    EntityType(it\collider, HIT_ITEM)
