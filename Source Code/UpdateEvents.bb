					...
					Else
						...
						If e\EventState = 0 Then
							If PlayerRoom = e\room Then
								...
								ChangeNPCTextureID(e\room\NPC[9], 15)
							...
							If IntroSFX(17)<>0 Then
								If EntityVisible(Curr173\Collider, Collider) Then
									If EntityInView(Curr173\obj, Camera) Then
										Msg = "ажмите "+KeyName(KEY_BLINK)+", чтобы моргнуть." ;Press ;to blink.
										MsgTimer = 70*4
							...
							ElseIf e\EventState > 230
								...
								For i = 1 To 2
									If Distance(EntityX(e\room\NPC[i]\Collider), EntityZ(e\room\NPC[i]\Collider), EntityX(e\room\Objects[i + 2], True), EntityZ(e\room\Objects[i + 2], True)) > 0.3 Then
										...
										;moveentity(e\room\npc(i).Collider, 0, 0, 0.015 * fs\FPSfactor[0])
				...
				If PlayerRoom = e\room Then
					...
					If e\EventState3=0 Then
						If Rand(2)=1 Then
							...
							e\room\Objects[1] = CopyEntity(o\NPCModelID[29])
			...
			Case "room895", "room895_106"
				...
				If PlayerRoom = e\room Then
					...
					If WearingNightVision > 0 Then
						...
						If (CoffinDistance < 4.0) And (hasBatteryFor895) And (Not Wearing714) Then
							...
							ElseIf Sanity < - 800 Then
								...
								If Rand(6) < 5 Then
									EntityTexture(at\OverlayID[3], at\OtherTextureID[Rand(5, 14)])
									For i% = 0 To MaxItemAmount - 1
										If (Inventory(i) <> Null) Then
											If (WearingNightVision = 1 And Inventory(i)\itemtemplate\tempname = "nvgoggles") Or (WearingNightVision = 2 And Inventory(i)\itemtemplate\tempname = "supernv") Or (WearingNightVision = 3 And Inventory(i)\itemtemplate\tempname = "finenvgoggles") Then
					...
					If WearingNightVision > 0 And (hasBatteryFor895) And EntityDistance(Collider, e\room\Objects[1]) < 10.0 Then
					    If e\Sound = 0 Then
							e\Sound = LoadSound_Strict("SFX\Room\895Chamber\NVG.ogg")
					    EndIf
				...
				If (Not Contained106) Then
					...
					Else
						...
						If dist<0.4 Then
							...
							;TurnEntity(e\room\NPC[0]\Collider,0,0,0.5*fs\FPSfactor[0])
			...
			Case "room2closets"
				...
				Else
					...
					Else
						...
						If BlinkTimer =-10 Then
							If e\EventState > 70*7.5 And e\EventState-fs\FPSfactor[0] =< 70*7.5 Then
								...
								If KillTimer => 0 Then 
									For i = 0 To 2
										If Distance(EntityX(Collider),EntityZ(Collider),EntityX(e\room\Objects[i],True),EntityZ(e\room\Objects[i],True)) < 250.0*RoomScale Then
											...
											CameraShake = 1.0
											Kill()
											DeathMSG = SubjectName$+" убит тесла-воротами в [УДАЛЕНО]." ;Subject D-9341 killed by the Tesla gate at [REDACTED].
										EndIf
									Next
								EndIf
								...
								ElseIf n\NPCtype = NPCtypeMTF2 Then
								    If Abs(EntityX(n\Collider)-EntityX(e\room\obj,True))<4.0 Then
										If Abs(EntityZ(n\Collider)-EntityZ(e\room\obj,True))<4.0 Then
											temp = True
											If e\EventState2 = 0 Then
												n\Sound = LoadSound_Strict("SFX\Character\MTF2\Tesla0.ogg")
												PlayMTFSound(n\Sound,n)
			...
			Case "room2tunnel"	
				...
				If PlayerRoom = e\room Then
					...
					;		End Select
							...
							HideEntity Meshes[i]
						Next
					;	Meshes[5]=CopyEntity(o\OBJTunnel[5])
					;	Meshes[5]=LoadRMesh("GFX\map\mt_elevator.rmesh",Null)
					;	HideEntity Meshes[5]
					;	Meshes[6]=CopyEntity(o\OBJTunnel[6])
			...
			Case "room3pitduck"
				...
				If PlayerRoom = e\room Then
					If e\room\Objects[2] = 0 Then
						e\room\Objects[2] =	CopyEntity(o\NPCModelID[31])
			...
			Case "room3pit1048"
				...
				If PlayerRoom = e\room Then
					If e\room\Objects[2] = 0 Then
						...
						Local imgPath$ = "GFX\items\1048\1048_"+Rand(1,26)+".png"
			...
			Case "room2servers"
				...
				If e\EventState = 0 Then
					If PlayerRoom = e\room Then
						...
						If e\Sound = 0 Then e\Sound = LoadSound_Strict("SFX\Character\Guard\096ServerRoom1.ogg")
						e\SoundCHN = PlaySound_Strict(e\Sound)
				...
				ElseIf e\EventState < 70*45
					...
					If e\room\NPC[0]<>Null Then
						...
						If AnimTime(Curr096\obj) > 25 And AnimTime(Curr096\obj)<150 Then
							...
							If e\room\dist > 0.0 Then
							    If e\Sound2 = 0 Then e\Sound2 = LoadSound_Strict("SFX\Character\Guard\096ServerRoom2.ogg")
							Else
							    If e\Sound2 = 0 Then e\Sound2 = LoadSound_Strict("SFX\Character\Guard\096ServerRoom3.ogg")
							EndIf
							e\SoundCHN2 = PlaySound_Strict(e\Sound2)
					...
					If temp = True Then 
						...
						Select e\EventState 
							...
							Case 25
								...
								tex=LoadTexture("GFX\NPCs\corpse.png")
								e\room\NPC[0]\texture = "GFX\NPCs\corpse.png"
					...
					If EntityY(Collider)<-4600*RoomScale Then
						...
						Else
							If e\EventState = 0 Then
							    ...
							    e\room\NPC[3]\State = 2                                                                                                                        ;> <--- ADDED (Jabka)
							...
							;DebugLog "WHERE IS IT?!"	
								...
								If dist < 0.6 Then
									...
									ElseIf e\EventState3>13*70 And e\EventState3-fs\FPSfactor[0]=<13*70
										...
										Else
										    Msg = "Вы начинаете вонзать ногти в свое запястье, но SCP-1033-RU защищает вас." ;You start pushing your nails into your wrist, but SCP-1033-RU protected you.
										    MsgTimer = 7*70
									        PlaySound_Strict LoadTempSound("SFX\SCP\012\Speech2.ogg")
									        Damage1033RU(10+(5*SelectedDifficulty\aggressiveNPCs))
                                        EndIf
									ElseIf e\EventState3>31*70 And e\EventState3-fs\FPSfactor[0]=<31*70
										...
										Else
									        Msg="Вы не можете вскрыть своё запястье." ;You can't open your left wrist
										    MsgTimer = 7*70
									        PlaySound_Strict LoadTempSound("SFX\SCP\012\Speech"+Rand(3,4)+".ogg")
									        Damage1033RU(10+(5*SelectedDifficulty\aggressiveNPCs))
                                       EndIf
									ElseIf e\EventState3>49*70 And e\EventState3-fs\FPSfactor[0]=<49*70
									    If I_1033RU\HP = 0
										 ...
										 Else
									        Msg="Вы не можете засунуть пальцы глубже рану." ;You can't push your fingers deeper.
										    MsgTimer = 8*70
                                            PlaySound_Strict LoadTempSound("SFX\SCP\012\Speech5.ogg")
									...
									ElseIf e\EventState3>85*70 And e\EventState3-fs\FPSfactor[0]=<85*70
									    If I_1033RU\HP = 0	
										    DeathMSG = SubjectName$+" найден в луже крови рядом с SCP-012. Похоже, что субъект разодрал в кровь свои запястья и " ;Subject D-9341 found in a pool of blood next to SCP-012. Subject seems to have ripped open his wrists and written three extra 
						...
						If e\room\NPC[0]=Null Then
							For n.NPCs = Each NPCs
								If n\texture = "GFX\npcs\scp035_victim.png" Then
									...
									e\room\NPC[0]\obj = CopyEntity(o\NPCModelID[33])
								...
								If EntityZ(Collider)>Min(EntityZ(e\room\Objects[7],True),EntityZ(e\room\Objects[8],True)) Then
									If EntityZ(Collider)<Max(EntityZ(e\room\Objects[7],True),EntityZ(e\room\Objects[8],True)) Then
										...
										If KillTimer < 0 And Bloodloss =>100 Then
											DeathMSG = SubjectName$+" найден мёртвым внутри камеры содержания SCP-035. " ;Class D Subject D-9341 found dead inside SCP-035's containment chamber. 
...
;								e\room\NPC[2]\State3 = e\room\NPC[2]\State3 + fs\FPSfactor[0]
;							Else
;								e\room\NPC[2]\State = 6
...
;						EndIf
					...
					ElseIf e\EventState < 10000
						...
						ElseIf e\EventState < 2000 Then ;3500
							...
							If IsStreamPlaying_Strict(e\SoundCHN)
								If Rand(3) = 1 Then
									EntityTexture(e\room\Objects[1], at\OtherTextureID[3])
							...
							Else
								...
								EndIf
								EntityTexture(e\room\Objects[1], at\OtherTextureID[4])
								ShowEntity (e\room\Objects[1])
								e\EventState = e\EventState + fs\FPSfactor[0]
							EndIf
						Else
				...
				If PlayerRoom = e\room And e\room\NPC[0]<>Null Then
					...
					ElseIf e\EventState = 1 ;bone broken
						...
						If e\EventState3 => 2500 Then
							...
							If e\EventState2 = 1 And e\EventState3-fs\FPSfactor[0] < 2500 Then
							...
							If e\EventState3-fs\FPSfactor[0] < 2500 Then 
						...
						Select e\EventState
							...
							Case 66
								...
								If AnimTime(e\room\Objects[3])>515 Then
									;MoveEntity(e\room\Objects[3], 0,-fs\FPSfactor[0]*0.001,0)
									;MoveEntity(e\room\Objects[4], 0,-fs\FPSfactor[0]*0.001,0)
									;MoveEntity(e\room\Objects[5], 0,-fs\FPSfactor[0]*0.001,0)
							...
							Case 67
								If (Rand(150)=1) Then
									...
									DeathMSG = DeathMSG + "[УДАЛЕНО]. Тело "+SubjectName2$+" было найдено внутри камеры. " ;[REDACTED]. The body of "+SubjectName$+" was discovered inside the chamber. "
				...
				If PlayerRoom = e\room Then
				    If EntityDistance(Collider, e\room\Objects[3]) < 0.9 Then
					    ...
					    If (Not Wearing714=1) Then
						    e\Sound = PlaySound_Strict(Ambient1123SFX%)
					    EndIf
				    EndIf
					...
					If e\EventState = 1 Then
						...
						nazi = CopyEntity(o\NPCModelID[28])
			...
			Case "testroom"
				...
				If e <> Null Then
					If PlayerRoom = e\room Then
						If e\EventState = 0 Then
							e\room\Objects[7] = CopyEntity(o\NPCModelID[29])
				    ...
				    If EntityY(Collider) < - 200.0 * RoomScale Then
					    ...
					    If e\EventState = 0 Then					
						    If Curr173\Idle < 2 And EntityDistance(Curr173\Collider,Collider) > HideDistance ;Just making sure that 173 is far away enough to spawn him to this room
							    PositionEntity Curr173\Collider, EntityX(e\room\Objects[3],True), - 416.0 * RoomScale, EntityZ(e\room\Objects[3],True),True
							    ResetEntity Curr173\Collider
						    EndIf											
						    e\EventState = 1
					    ElseIf e\EventState = 1
							    ...
							    If e\EventState2 = 0 Then
								    ...
								    If EntityDistance(Curr173\Collider,e\room\Objects[4])<3.0
									    If (BlinkTimer<-10 Or (Not EntityInView(Curr173\obj,Camera))) And Curr173\Idle = 0 Then
										    PositionEntity Curr173\Collider, EntityX(e\room\Objects[4],True), - 416.0 * RoomScale, EntityZ(e\room\Objects[4],True),True
										    ResetEntity Curr173\Collider
			...
			Case "682roar"
				...
				ElseIf PlayerRoom\RoomTemplate\Name <> "pocketdimension" And PlayerRoom\RoomTemplate\Name <> "room860" And PlayerRoom\RoomTemplate\Name <> "room1123" And PlayerRoom\RoomTemplate\Name <> "dimension1499" 
					...
					If e\EventState < 17*70 Then
						If	e\EventState+fs\FPSfactor[0] => 17*70 Then LoadEventSound(e,"SFX\SCP\682\Roar.ogg") : e\SoundCHN = PlaySound_Strict(e\Sound) ;e\Sound = LoadSound_Strict("SFX\SCP\682\Roar.ogg")
					...
					If e\EventState > 0 Then
						...
						If Distance(EntityX(Collider), EntityZ(Collider), EntityX(e\room\Objects[2], True), EntityZ(e\room\Objects[2], True)) < (170.0 * RoomScale) Then
							...
							If setting = "rough" Or setting = "coarse" Then
								If e\EventState > 70 * 2.6 And e\EventState - fs\FPSfactor[1] < 70 * 2.6 Then PlaySound_Strict Death914SFX
							EndIf
							...
							If e\EventState > 70 * 3 Then
								Select setting
									Case "rough"
										...
										BlinkTimer = -10
										If e\SoundCHN <> 0 Then StopChannel e\SoundCHN
										DeathMSG = Chr(34)+"В отсеке вывода SCP-914 был найден сильно изуродованный труп. После анализа ДНК труп был идентифицирован как "+SubjectName$+". " ;A heavily mutilated corpse found inside the output booth of SCP-914. DNA testing identified the corpse as Class D "+SubjectName$+". "
			...
			Case "room4tunnels"
				...
				If e\room\dist < 10 Then
				    If e\EventState = 0
					    ...
					    SetNPCFrame e\room\NPC[0], 19
					    e\room\NPC[0]\State = 8
					    TurnEntity e\room\NPC[0]\Collider, 0, e\room\angle + 290, 0
					    e\room\NPC[0]\IsDead = True
                        e\EventState = 1
                    Else
                        RemoveEvent(e)
							...
							If (IsItemGoodFor1162(itt)) Then
								Select Inventory(e\EventState2)\itemtemplate\tempname
									...
									Case "scp198"
									    ...
									    DeathMSG = DeathMSG + " Похоже, субъект взаимодействовал с SCP-198" ;Most likely he interaction with SCP-198
									    DeathMSG = DeathMSG + " и SCP-1162." ;and SCP-1162.
									    PlaySound_Strict LoadTempSound("SFX\SCP\1162\BodyHorrorExchange"+Rand(1,4)+".ogg")
									    LightFlash = 5.0
									    Kill(True)
									    Msg = "SCP-198 отлип от вашего тела, он теперь у вас серьёзные травмы." ;SCP-198 is now detached from your body, but you have high injuries.
									    MsgTimer = 70*5
							            Exit
									    DebugLog "scp198"
									Case "scp357"
									    If I_357\Timer >= 56.0 Then
									        ...
									        e\EventState = e\EventState + 1.0
									        e\EventState3 = 0.0
									        DeathMSG = SubjectName$+". Причина смерти: неизвестна" ;Cause of death: Unknown.
									        BlurTimer = 1000
									        Kill(True)
									        Msg = "SCP-357 отлип от вашего тела, он теперь у вас серьёзные травмы." ;SCP-357 is now detached from your body, but you have high injuries.
									        MsgTimer = 70*5
									        Exit
									    Else
									        RemoveItem(Inventory(e\EventState2))
					...
					ElseIf e\EventState2 = 3
						...
						If e\room\NPC[0]\PathStatus <> 1
							...
							If e\room\NPC[0]\PathTimer# = 0.0
								;e\room\NPC[0]\PathTimer# = e\room\NPC[0]\PathTimer# + fs\FPSfactor[0]
			...
			Case "medibay"
				...
				If EntityDistance(e\room\Objects[4],Collider) < 0.75 Then
				    DrawHandIcon = True
		            If MouseHit1 Then
					    Msg = "Вы чувствуете холодный ветерок." ;"You feel a cold breeze next to your body.
					    MsgTimer = 70*5
			            Injuries = 0
			            Bloodloss = 0
			            PlaySound_Strict(LoadTempSound("SFX\SCP\Joke\KRYA.ogg"))
			            GiveAchievement(AchvDuck)
			   	    EndIf
				EndIf
					...
					If e\room\dist < 8
						...
						If e\room\NPC[1] = Null
							If EntityDistance(e\room\NPC[0]\Collider,Collider) < 2.5
								e\room\NPC[1] = CreateNPC(NPCtypeTentacle, EntityX(e\room\NPC[0]\Collider), 0.17, EntityZ(e\room\NPC[0]\Collider))
								RotateEntity e\room\NPC[1]\Collider, 0, e\room\angle, 0
								MoveEntity e\room\NPC[1]\Collider, 0, 0, 0.6
							EndIf
						ElseIf e\room\dist > HideDistance And e\room\NPC[1]\IsDead = False
							If e\room\NPC[1] <> Null
								RemoveNPC(e\room\NPC[1])
								e\room\NPC[1] = Null
							EndIf
			...
			Case "medibay2"
				...
				If PlayerRoom = e\room Then
					If e\EventState = 0 Then 
						n.NPCs = CreateNPC(NPCtype0082, EntityX(e\room\Objects[5],True),EntityY(e\room\Objects[5],True),EntityZ(e\room\Objects[5],True))
			...
			Case "room457"
				...
				If PlayerRoom = e\room Then
				    ...
				    ;elevators
				    e\EventState4 = UpdateElevators(e\EventState4,e\room\RoomDoors[0], e\room\RoomDoors[2], e\room\Objects[4], e\room\Objects[6], e)
					e\EventState3 = UpdateElevators(e\EventState3,e\room\RoomDoors[1], e\room\RoomDoors[3], e\room\Objects[5], e\room\Objects[7], e)
					  
					If EntityY(Collider) < - 2676.0 * RoomScale
						...
						Else
							e\EventState = 1.0 
							If e\room\NPC[0]\State = 2 And (Not chs\Notarget) 
								e\room\NPC[0]\State = 0
								If (Not Contained457) Then
								    PositionEntity e\room\NPC[0]\Collider, EntityX(e\room\Objects[0], True), EntityY(e\room\Objects[0], True) - 0.5, EntityZ(e\room\Objects[0], True)
								Else
								    PositionEntity e\room\NPC[0]\Collider, EntityX(e\room\Objects[9], True), EntityY(e\room\Objects[9], True) - 0.4, EntityZ(e\room\Objects[9], True)
                                EndIf
							EndIf
							If e\EventState > 0.0 And (Not chs\Notarget)
								e\room\NPC[0]\TargetEnt = e\room\Objects[(Int(e\EventState-1))]
							EndIf
							If e\room\NPC[0]\PathStatus <> 1
								e\EventState = 1.0
							EndIf
							If EntityDistance(e\room\Objects[9], e\room\NPC[0]\Collider) < 6.0 And e\room\RoomDoors[4]\open = False And e\room\RoomDoors[5]\open = False And e\room\RoomDoors[6]\open = False Then
							    Contained457 = True
							Else
							    Contained457 = False
							EndIf	
		    ...
		    Case "room009"
				...
				If e\Sound = 0 Then e\Sound = LightBlinkSFX%
				...
				If PlayerRoom=e\room Then
					...
					If e\EventState >= 300.0 And e\EventState3 = 0.0 Then
						...
						PlaySound TempSound
						If Rand(2) = 1 Then
							Msg = "Вы чувствуете жгучую боль в правой ноге." ;You feel a burning pain in your right leg.
						Else
							Msg = "Вы чувствуете жгучую боль в левой ноге." ;You feel a burning pain in your left leg.
						EndIf
						MsgTimer = 70*5
						CameraShake = 5.0
					EndIf
				...
				If e\EventState >= 4290.0 And e\EventState3 = 1.0 Then
			        PlaySound_Strict LoadTempSound("SFX\SCP\009\ice-cracking.ogg")
					PlaySound TempSound
					e\EventState3 = 2.0
				EndIf
				...
				If e\EventState >= 4502.0 And e\EventState3 = 2.0 Then
					...
					Else
						DeathMSG = "Кристаллизированный субъект класса D был обнаружен. Причиной смерти послужил" ;A crystallized D-class subject was found. Death caused by
			...
			Case "room096"
			    ...
			    If e\room\dist > 0 And e\room\dist < 10 Then
			        If e\EventState2 = 0 Then
			            e\Sound = LoadSound_Strict("SFX\Door\DoorClose079.ogg")
						e\SoundCHN = PlaySound2(e\Sound, Camera, e\room\Objects[2], 100, 0.18)
                        e\EventState2 = 1
                    EndIf
                EndIf
				If PlayerRoom = e\room Then
				    If e\EventState = 0 Then
						...
						If Curr096 = Null And RemoteDoorOn Then
							Curr096 = CreateNPC(NPCtype096, EntityX(e\room\Objects[0],True), EntityY(e\room\Objects[0],True) + 0.1,EntityZ(e\room\Objects[0],True))
					        RotateEntity Curr096\Collider, - 180 , e\room\angle + 90, 0, True
					        ResetEntity Curr096\Collider
					        ...
					        ;blood decal
						    de.Decals = CreateDecal(17, EntityX(e\room\Objects[1],True), EntityY(e\room\Objects[1],True) + 0.01,EntityZ(e\room\Objects[1],True), 90, 90, 0)
					        de\Size = 0.07 : de\SizeChange = 0.001 : EntityAlpha(de\obj, 0.8) : UpdateDecals()
				
					        ;playing event sound #2
						    e\Sound2 = LoadSound_Strict("SFX\Room\096Chamber\096ChamberEvent.ogg")
						    e\SoundCHN2 = PlaySound_Strict(e\Sound2)
						
						    ;items
						    it = CreateItem("Ключ-карта 2-го уровня", "key2", EntityX(e\room\Objects[3], True), EntityY(e\room\Objects[3], True) + 0.4, EntityZ(e\room\Objects[3], True)) ;Level 2 Key Card
					        EntityType(it\collider, HIT_ITEM)
					
					        it = CreateItem("Неизвестный документ", "paper", EntityX(e\room\Objects[4], True), EntityY(e\room\Objects[4], True) + 0.4, EntityZ(e\room\Objects[4], True)) ;Unknown Document
					        EntityType(it\collider, HIT_ITEM)
						...
						Else
						    ...
						    RemoveEvent(e)
						EndIf
				    ElseIf e\EventState < 70 * 30
				        ;light
					    If Rand(200) < 5 And PlayerRoom = e\room Then
					        If (Not WearingNightVision) Then LightBlink = Rnd(3.0,6.0)
						    If Rand(5) = 1 Then PlaySound2(IntroSFX(Rand(10,12)), Camera, e\room\obj, 8.0, Rnd(0.1, 0.3))
					    EndIf
					    Curr096\State = -1
					    e\EventState = Min(e\EventState + fs\FPSfactor[0], 70 * 31)
					    ;unlocked door
					    If e\EventState > 70 * 28 And e\EventState < 70 * 30 Then
					        e\room\RoomDoors[0]\locked = False
					        e\room\RoomDoors[2]\locked = False
					        Curr096\State = 0
					        RemoveEvent(e)
            ...
            Case "room409"
				;[Block]
				If PlayerRoom = e\room Then
					If EntityY(Collider) < - 2999 *RoomScale Then
				        ...
				        ElseIf e\EventState = 1.0 Then 	
					        ...
					        EndIf
					        ;touch the SCP-409
					        If EntityDistance(e\room\Objects[4],Collider)<0.45 Then
					            If I_409\Timer < 1 Then
						            DrawHandIcon = True
						            If MouseHit1 Then
						                Msg = "Вы коснулись SCP-409." ;You touched the SCP-409.
						                MsgTimer = 70*6
						                BlurTimer = 2000
			    ...
			    If (Not Contained106) Then 
			        If PlayerRoom = e\room
					    ...
					    If KeyDistance < 1.5 Then
				            If e\EventState = 0 And (Not diceroll = 2) Then
				                PlaySound_Strict(HorrorSFX(10))
			...
			Case "exit1"
				...
				If PlayerRoom = e\room Then
					...
					If EntityY(Collider)>1040.0*RoomScale Then
						...
						Else
							...
							If e\EventState > 26.5*70 Then
								If e\room\Objects[12] = 0 Then
									e\room\Objects[12] = CopyEntity(o\NPCModelID[34])
						...
						;EntityAlpha at\OverlayID[0], 1.0-dist
						HideEntity at\OverlayID[0]
								...
								ElseIf e\EventState3 = 1.0
									...
									If Abs(EntityY(Collider)-EntityY(e\room\Objects[17],True))<1.0 Then
										If Distance(EntityX(Collider),EntityZ(Collider),EntityX(e\room\Objects[17],True),EntityZ(e\room\Objects[17],True)) < 7.0 And e\EventState4 = 0 Then
											...
											e\SoundCHN = PlaySound2(LoadTempSound("SFX\Ending\GateA\Bell1.ogg"), Camera, e\room\NPC[9]\Collider)
